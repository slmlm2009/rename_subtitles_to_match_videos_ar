...
def rename_subtitles_to_match_videos():
    ...
    return renamed_count, movie_mode_detected

def export_analysis_to_csv(renamed_count=0, movie_mode=False):
    """
    ...
    - Proper CSV table showing all files with detected episodes and actions
    ...
    for subtitle in sorted(subtitle_files):
        ...
        if episode_string:
            ...
            # Determine new name based on matching
            if episode_string in temp_video_dict:
            ...
            detected_episode = "Movie"
            # Find the video file for movie mode
    ...
    unmatched_subtitles = len([s for s in subtitle_files if get_episode_number(s) and get_episode_number(s) not in temp_video_dict.keys()])
    ...
    with open(csv_path, 'w', encoding='utf-8', newline='') as csvfile:
        ...
        csvfile.write(f"# Unmatched Videos: {unmatched_videos}\n")
        csvfile.write(f"# Unmatched Subtitles: {unmatched_subtitles}\n")
        csvfile.write(f"# Unidentified Files: {unidentified_count}\n")
            ...
            for episode in sorted(not_found_episodes):
                if episode != "(None)":
                    ...
                    if episode in temp_video_dict:
                        ...
                        csvfile.write(f"# {episode} -> Has Subtitle | Missing: Video\n")
            csvfile.write("#\n")
        
        if unidentified_files:
            csvfile.write("# UNIDENTIFIED FILES:\n")
            if movie_mode:
