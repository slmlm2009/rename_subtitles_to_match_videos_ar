...
def rename_subtitles_to_match_videos():
    ...
    for video in sorted_video_files_for_context:
        ...
        if episode_string:
            ...
            if season and episode:
                ...
                # Only establish the episode pattern and video reference for the first video alphabetically
                ...
                # For subsequent videos with the same episode, just store in temp_video_dict if not exists
    ...
    # Use a dictionary that preserves the first occurrence for each pattern to ensure alphabetical priority
    video_dict = {}
    remaining_video_files = []
    
    # Process videos in alphabetical order to ensure consistent prioritization
    sorted_video_files_for_dict = sorted(video_files)
    for video in sorted_video_files_for_dict:
        ...
        if ep:
            ...
            if season and episode:
                ...
                if key in video_episodes and video_episodes[key] != ep:
                    ...
                    # Only store this video if no other video has been stored for this adjusted pattern
                    if video_episodes[key] not in video_dict:
                        video_dict[video_episodes[key]] = video
                else:
                    # Only store this video if no other video has been stored for this pattern
                    if ep not in video_dict:
                        video_dict[ep] = video
            else:
                # Only store this video if no other video has been stored for this pattern
                if ep not in video_dict:
