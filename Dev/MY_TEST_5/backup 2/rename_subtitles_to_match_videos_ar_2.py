...
def rename_subtitles_to_match_videos():
    ...
    print(f"FILES FOUND: {len(video_files)} videos | {len(subtitle_files)} subtitles")
    print("="*60)
    
    if video_files:
        print(f"Videos: {video_files[:3]}{'...' if len(video_files) > 3 else ''}")
    if subtitle_files:
        print(f"Subtitles: {subtitle_files[:3]}{'...' if len(subtitle_files) > 3 else ''}")
    print()
    ...
    if video_episodes:
        ...
        print("-" * 40)
        print("EPISODE PATTERNS DETECTED FROM VIDEOS: {list(video_episodes.values())[:10]}{'...' if len(video_episodes) > 10 else ''}")
    print()
    ...
    print("="*60)
    if renamed_count > 0:
        print(f"SUCCESS: {renamed_count} file{'s' if renamed_count != 1 else ''} renamed!")
    else:
...
def export_analysis_to_csv():
    ...
    print(f"\nAnalysis exported to: {txt_path}")
    print(f"Exported {len(identified_files) + len(unidentified_files)} file records to analysis file.")
